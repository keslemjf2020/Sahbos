...
import { postsFull } from "../posts-full";
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f]">
      <div className="border-b border-white/[0.04]"><div className="max-w-3xl mx-auto px-6 py-16">
        ...
        <span className="inline-block text-[10px] px-2 py-0.5 rounded-full bg-cyan-500/10 text-cyan-400 font-medium uppercase tracking-wider mb-4">{post.cat}</span>
      ...
      <article className="max-w-3xl mx-auto px-6 py-12">
        <div className="prose prose-invert max-w-none prose-headings:text-white prose-h2:text-2xl prose-h2:font-bold prose-h2:mt-10 prose-h3:text-lg prose-h3:font-semibold prose-p:text-slate-300 prose-p:leading-relaxed prose-a:text-cyan-400 prose-strong:text-white">
          {post.content.split("\\n").map((line, i) => {
            ...
            if (line.startsWith("- ")) return <li key={i} className="text-slate-300">{line.replace("- ", "")}</li>;
