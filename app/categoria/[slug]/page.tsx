...
export default function Category({ params }: { params: { slug: string } }) {
  ...
  for (const [key, val] of Object.entries(catData)) {
    ...
    if ((post.c || "IA").toLowerCase() === slug.toLowerCase()) {
      items.push({ slug: key, t: post.t || key, d: post.d || "", c: post.c || "IA" });
    }
  }
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f]">
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        {items.length === 0 ? (
          <div className="text-center py-12"><p className="text-slate-500 text-lg mb-2">Nenhum artigo.</p><Link href="/mapa-do-site" className="text-cyan-400 hover:underline text-sm">Ver Mapa do Site →</Link></div>
        ) : (
